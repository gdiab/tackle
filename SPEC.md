...
- **Sandcastle: dependency or reference?** The Phase 0 spike decides whether Tackle builds on `@ai-hero/sandcastle`, cribs its contract into own adapters, or rejects it with a reason. Unresolved until the spike runs; the spec's `TurnResult` shape should be treated as provisional against that comparison.
