...
- **Test-first, gated on a dependency map.** Build steps that add code without a failing test first get rejected, but only once a source-to-test dependency map exists for the repo. This caveat is load-bearing, not pedantic: TDAD (test-driven agentic development research) measured that generic "write a failing test first" instructions *without* targeted test context pushed regressions to 9.94%, worse than the 6.08% no-TDD baseline, while the dependency map dropped them to 1.82%. (Provenance, verified 2026-07-02 against the full paper, arXiv:2603.17973v2: the numbers are exact — Phase 1, 100 SWE-bench Verified instances, Qwen3-Coder 30B 4-bit, test-level pass-to-pass regression rate, 562/9,245 vs 799/8,040 vs 155/8,536. Known limits the gate design inherits: no significance tests, single run per arm, one small quantized model — the authors themselves warn "frontier models may not exhibit the same TDD-prompting paradox" — Python-only on well-tested repos, and the 1.82% arm is map *plus* TDD instructions, never map alone. The map also raised the abstention rate (empty patches 26% vs 14%), so blocking has a throughput cost. Directionally solid, externally thin: the advisory-vs-blocking split stands as "consistent with TDAD" rather than proven, and Tackle's own regression telemetry is what confirms or falsifies it on our models and stack.) A blocking gate that makes outcomes worse is negative value, so the rule is advisory-only until the map exists and blocking once it does. The map is the "lightweight agent skill / static text file queried at runtime" that TDAD describes: its *builder* is stack-specific and ships in the stack profile, the *map itself* is built and cached per repo from the import graph plus coverage data (the coverage augmentation is Tackle's extension — TDAD's own map is static AST plus test-naming heuristics, no coverage). The JS/TS map-builder is a named Phase 0 deliverable, not a Phase 2 nicety, precisely so the test-first gate never ships in its harmful form.
...
2. **Harvest, then research.** Before generating anything, it harvests skills that already exist in George's repos. This isn't hypothetical: the Arbatash repo carries substantive SwiftUI skills (`swiftui-animation`, `swiftui-gestures`, `swiftui-layout-components`, `swiftui-patterns` — 300–480 lines each, iOS 26 / Swift 6.2, MV architecture explicit in `swiftui-patterns`), vendored from the public `dpearson2699/swift-ios-skills` repo rather than homegrown. Those get ingested as the Swift starting point rather than reinvented — though the harvest should point at the upstream repo, which is complete (~85 skills, including the `swiftui-navigation` the vendored set references but lacks) where Arbatash's copy is partial. Only after harvesting does a subagent research what's still missing: test pyramid and runner, lint and format and build and test commands, mutation tooling availability, SAST options, common smells.
...
Swift doesn't start from zero thanks to the Arbatash skills, which is lucky, because Swift is otherwise where this loop is weakest. The generated leaves with the least tooling support (mutation testing especially) will surface here first. The Arbatash skills also de-risk the harvest step: they're a real test of whether ingesting existing repo skills produces a usable profile or just noise. (Verified 2026-07-02: four of the five claimed skills exist in Arbatash at `.claude/skills/` and are substantive, but `swiftui-navigation` was never vendored — it lives only upstream in `dpearson2699/swift-ios-skills` — two skills have dangling `references/` links from partial vendoring, and "accessibility baked in" is real only for `swiftui-animation`. The harvest step should treat Arbatash as the pointer and the upstream repo as the source. Arbatash itself targets iOS 17+ and tells agents to ignore iOS 26-only APIs, so the iOS 26 framing is the skills' target, not the host repo's.)
...
- 2026-07-02 (verification chores): Closed both Phase 0 verification chores. TDAD (arXiv:2603.17973v2, full read): the 6.08%/9.94%/1.82% numbers are exact and the mapless-TDD-harms direction is real, but the evidence is one unreviewed preprint, one small quantized model, single run, Python-only, and the authors doubt the paradox transfers to frontier models — hedge rewritten at the test-first gate; coverage-augmented map noted as Tackle's extension beyond TDAD's static-AST approach. Arbatash: 4/5 skills exist and are substantive but are a partial vendor of `dpearson2699/swift-ios-skills` (~85 skills upstream, including the missing `swiftui-navigation`); harvest step repointed at upstream with Arbatash as the pointer.
